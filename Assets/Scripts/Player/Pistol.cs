...
public class Pistol : MonoBehaviour
{
    ...
    private void Shoot()
    {
        ...
        m_fireTimer = 0;
        if (Physics.Raycast(m_playerCameraTF.position, m_playerCameraTF.forward, out RaycastHit hit, 1000f,
                m_gunLayerMask))
        {
    ...
    {
        GameObject muzzleVFX = Instantiate(m_muzzleVFX, m_pistolMuzzleTF.position, Quaternion.identity);
    ...
    {
        if (!Physics.Raycast(
                m_playerCameraTF.position,
                m_playerCameraTF.forward,
                out RaycastHit hit,
                1000f,
                m_gunLayerMask
            )) return;
        ...
        m_pistolTF.rotation =
            Quaternion.Slerp(m_pistolTF.rotation, lookRotation, Time.deltaTime / m_pistolRotationSpeed);
    }
}
